// Command Solardemo-cli is a CLI + REPL for the Solardemo SDK,
...
const prompt = "solardemo"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "moon planet"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SOLARDEMO_APIKEY (api key), SOLARDEMO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SOLARDEMO_APIKEY carries the API key and
	// SOLARDEMO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SOLARDEMO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SOLARDEMO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
