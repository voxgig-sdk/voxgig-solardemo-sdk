package voxgig.solardemosdk.core;

import java.util.Map;

import voxgig.solardemosdk.utility.Json;

/** Static SDK configuration and by-name feature construction. */
@SuppressWarnings({"unchecked"})
public final class Config {

  private Config() {}

  public static Map<String, Object> makeConfig() {
    return (Map<String, Object>) Json.parse(configJson());
  }

  // SHARED CONFIG (sdkgen rung L2).
  //
  // The SDK reads the config on every request and never writes to it, so one
  // instance is shared by every client rather than rebuilt per client - the
  // difference between parsing the embedded JSON once and once per client.
  //
  // Initialization-on-demand holder: the JLS guarantees the class initializer
  // runs once, lazily, and safely under concurrency, with no locking on the
  // read path.
  private static final class SharedHolder {
    static final Map<String, Object> VALUE = makeConfig();
  }

  // The process-wide config, built once on first use.
  //
  // The returned map is SHARED: treat it as read-only. Callers that need to
  // mutate should use makeConfig, which always parses a fresh copy.
  public static Map<String, Object> sharedConfig() {
    return SharedHolder.VALUE;
  }

  public static Feature makeFeature(String name) {
    switch (name) {
      case "test":
        return new voxgig.solardemosdk.feature.TestFeature();
      default:
        return new voxgig.solardemosdk.feature.BaseFeature();
    }
  }

  private static String configJson() {
    StringBuilder b = new StringBuilder();
    b.append("{");
    b.append(" \"main\": {");
    b.append("  \"name\": \"Solardemo\",");
    b.append("  \"slug\": \"solardemo\",");
    b.append("  \"version\": \"0.1.0\",");
    b.append("  \"target\": \"java\"");
    b.append(" },");
    b.append(" \"feature\": {");
    b.append("  \"test\": {");
    b.append("   \"options\": {");
    b.append("    \"active\": false");
    b.append("   },");
    b.append("   \"transport\": \"base\"");
    b.append("  }");
    b.append(" },");
    b.append(" \"options\": {");
    b.append("  \"base\": \"http://localhost:8901\",");
    b.append("  \"headers\": {");
    b.append("   \"content-type\": \"application/json\"");
    b.append("  },");
    b.append("  \"entity\": {");
    b.append("   \"moon\": {},");
    b.append("   \"planet\": {}");
    b.append("  }");
    b.append(" },");
    b.append(" \"entity\": {");
    b.append("  \"moon\": {");
    b.append("   \"fields\": [");
    b.append("    {");
    b.append("     \"format\": \"float\",");
    b.append("     \"name\": \"diameter\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$NUMBER`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"id\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"kind\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"name\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"planet_id\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    }");
    b.append("   ],");
    b.append("   \"name\": \"moon\",");
    b.append("   \"op\": {");
    b.append("    \"create\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"create\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"planet_id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"POST /api/planet/{planet_id}/moon\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"requestBody\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"planet_id\\\":{\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"planet_id\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"required\\\":true},\\\"responses\\\":{\\\"201\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"planet_id\\\":{\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"planet_id\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"description\\\":\\\"Created\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"POST\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}/moon\",");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"planet_id\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"moon\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"exist\": [");
    b.append("         \"planet_id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{planet_id}\",");
    b.append("        \"moon\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    },");
    b.append("    \"list\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"list\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"planet_id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"GET /api/planet/{planet_id}/moon\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"responses\\\":{\\\"200\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"items\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"planet_id\\\":{\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"planet_id\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"},\\\"type\\\":\\\"array\\\"}}},\\\"description\\\":\\\"OK\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"GET\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}/moon\",");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"planet_id\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"moon\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"exist\": [");
    b.append("         \"planet_id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{planet_id}\",");
    b.append("        \"moon\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    },");
    b.append("    \"load\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"load\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"id\",");
    b.append("          \"orig\": \"moon_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         },");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"planet_id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"GET /api/planet/{planet_id}/moon/{moon_id}\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}},{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"moon_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"responses\\\":{\\\"200\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"planet_id\\\":{\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"planet_id\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"description\\\":\\\"OK\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"GET\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}/moon/{moon_id}\",");
    b.append("       \"rename\": {");
    b.append("        \"param\": {");
    b.append("         \"moon_id\": \"id\"");
    b.append("        }");
    b.append("       },");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"planet_id\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"moon\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"id\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"exist\": [");
    b.append("         \"id\",");
    b.append("         \"planet_id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{planet_id}\",");
    b.append("        \"moon\",");
    b.append("        \"{id}\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    },");
    b.append("    \"remove\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"remove\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"id\",");
    b.append("          \"orig\": \"moon_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         },");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"planet_id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"DELETE /api/planet/{planet_id}/moon/{moon_id}\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}},{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"moon_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"responses\\\":{\\\"204\\\":{\\\"description\\\":\\\"No Content\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"DELETE\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}/moon/{moon_id}\",");
    b.append("       \"rename\": {");
    b.append("        \"param\": {");
    b.append("         \"moon_id\": \"id\"");
    b.append("        }");
    b.append("       },");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"planet_id\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"moon\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"id\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"exist\": [");
    b.append("         \"id\",");
    b.append("         \"planet_id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{planet_id}\",");
    b.append("        \"moon\",");
    b.append("        \"{id}\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    },");
    b.append("    \"update\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"update\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"id\",");
    b.append("          \"orig\": \"moon_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         },");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"planet_id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"PUT /api/planet/{planet_id}/moon/{moon_id}\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}},{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"moon_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"requestBody\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"planet_id\\\":{\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"planet_id\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"required\\\":true},\\\"responses\\\":{\\\"200\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"planet_id\\\":{\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"planet_id\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"description\\\":\\\"OK\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"PUT\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}/moon/{moon_id}\",");
    b.append("       \"rename\": {");
    b.append("        \"param\": {");
    b.append("         \"moon_id\": \"id\"");
    b.append("        }");
    b.append("       },");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"planet_id\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"moon\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"id\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"exist\": [");
    b.append("         \"id\",");
    b.append("         \"planet_id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{planet_id}\",");
    b.append("        \"moon\",");
    b.append("        \"{id}\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    }");
    b.append("   },");
    b.append("   \"relations\": {");
    b.append("    \"ancestors\": [");
    b.append("     [");
    b.append("      \"planet\"");
    b.append("     ]");
    b.append("    ]");
    b.append("   }");
    b.append("  },");
    b.append("  \"planet\": {");
    b.append("   \"fields\": [");
    b.append("    {");
    b.append("     \"format\": \"float\",");
    b.append("     \"name\": \"diameter\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$NUMBER`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"forbidReason\",");
    b.append("     \"readOnly\": true,");
    b.append("     \"short\": \"Why the planet is forbidden, carried from the forbid action's `why`.\",");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"forbidState\",");
    b.append("     \"readOnly\": true,");
    b.append("     \"short\": \"Set by the forbid action, and absent until it first runs.\",");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"id\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"kind\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"name\",");
    b.append("     \"req\": true,");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    },");
    b.append("    {");
    b.append("     \"name\": \"terraformState\",");
    b.append("     \"readOnly\": true,");
    b.append("     \"short\": \"Set by the terraform action, and absent until it first runs.\",");
    b.append("     \"type\": \"`$STRING`\"");
    b.append("    }");
    b.append("   ],");
    b.append("   \"name\": \"planet\",");
    b.append("   \"op\": {");
    b.append("    \"create\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"create\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"POST /api/planet/{planet_id}/forbid\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"requestBody\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"forbid\\\":{\\\"type\\\":\\\"boolean\\\"},\\\"why\\\":{\\\"type\\\":\\\"string\\\"}},\\\"type\\\":\\\"object\\\"}}},\\\"required\\\":true},\\\"responses\\\":{\\\"200\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"ok\\\":{\\\"type\\\":\\\"boolean\\\"},\\\"state\\\":{\\\"type\\\":\\\"string\\\"}},\\\"type\\\":\\\"object\\\"}}},\\\"description\\\":\\\"OK\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"POST\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}/forbid\",");
    b.append("       \"rename\": {");
    b.append("        \"param\": {");
    b.append("         \"planet_id\": \"id\"");
    b.append("        }");
    b.append("       },");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"id\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"forbid\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"$action\": \"forbid\",");
    b.append("        \"exist\": [");
    b.append("         \"id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{id}\",");
    b.append("        \"forbid\"");
    b.append("       ]");
    b.append("      },");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"POST /api/planet/{planet_id}/terraform\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"requestBody\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"start\\\":{\\\"type\\\":\\\"boolean\\\"},\\\"stop\\\":{\\\"type\\\":\\\"boolean\\\"}},\\\"type\\\":\\\"object\\\"}}},\\\"required\\\":true},\\\"responses\\\":{\\\"200\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"ok\\\":{\\\"type\\\":\\\"boolean\\\"},\\\"state\\\":{\\\"type\\\":\\\"string\\\"}},\\\"type\\\":\\\"object\\\"}}},\\\"description\\\":\\\"OK\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"POST\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}/terraform\",");
    b.append("       \"rename\": {");
    b.append("        \"param\": {");
    b.append("         \"planet_id\": \"id\"");
    b.append("        }");
    b.append("       },");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"id\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"terraform\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"$action\": \"terraform\",");
    b.append("        \"exist\": [");
    b.append("         \"id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{id}\",");
    b.append("        \"terraform\"");
    b.append("       ]");
    b.append("      },");
    b.append("      {");
    b.append("       \"args\": {},");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"POST /api/planet\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[],\\\"protocol\\\":\\\"http\\\",\\\"requestBody\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"forbidReason\\\":{\\\"description\\\":\\\"Why the planet is forbidden, carried from the forbid action's `why`. Absent while the planet is allowed.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"forbidState\\\":{\\\"description\\\":\\\"Set by the forbid action, and absent until it first runs. One of allowed or forbidden.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"terraformState\\\":{\\\"description\\\":\\\"Set by the terraform action, and absent until it first runs. One of idle, terraforming or complete.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"required\\\":true},\\\"responses\\\":{\\\"201\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"forbidReason\\\":{\\\"description\\\":\\\"Why the planet is forbidden, carried from the forbid action's `why`. Absent while the planet is allowed.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"forbidState\\\":{\\\"description\\\":\\\"Set by the forbid action, and absent until it first runs. One of allowed or forbidden.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"terraformState\\\":{\\\"description\\\":\\\"Set by the terraform action, and absent until it first runs. One of idle, terraforming or complete.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"description\\\":\\\"Created\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"POST\",");
    b.append("       \"orig\": \"/api/planet\",");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {},");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    },");
    b.append("    \"list\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"list\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {},");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"GET /api/planet\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[],\\\"protocol\\\":\\\"http\\\",\\\"responses\\\":{\\\"200\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"items\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"forbidReason\\\":{\\\"description\\\":\\\"Why the planet is forbidden, carried from the forbid action's `why`. Absent while the planet is allowed.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"forbidState\\\":{\\\"description\\\":\\\"Set by the forbid action, and absent until it first runs. One of allowed or forbidden.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"terraformState\\\":{\\\"description\\\":\\\"Set by the terraform action, and absent until it first runs. One of idle, terraforming or complete.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"},\\\"type\\\":\\\"array\\\"}}},\\\"description\\\":\\\"OK\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"GET\",");
    b.append("       \"orig\": \"/api/planet\",");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {},");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    },");
    b.append("    \"load\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"load\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"GET /api/planet/{planet_id}\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"responses\\\":{\\\"200\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"forbidReason\\\":{\\\"description\\\":\\\"Why the planet is forbidden, carried from the forbid action's `why`. Absent while the planet is allowed.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"forbidState\\\":{\\\"description\\\":\\\"Set by the forbid action, and absent until it first runs. One of allowed or forbidden.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"terraformState\\\":{\\\"description\\\":\\\"Set by the terraform action, and absent until it first runs. One of idle, terraforming or complete.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"description\\\":\\\"OK\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"GET\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}\",");
    b.append("       \"rename\": {");
    b.append("        \"param\": {");
    b.append("         \"planet_id\": \"id\"");
    b.append("        }");
    b.append("       },");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"id\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"exist\": [");
    b.append("         \"id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{id}\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    },");
    b.append("    \"remove\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"remove\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"DELETE /api/planet/{planet_id}\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"responses\\\":{\\\"204\\\":{\\\"description\\\":\\\"No Content\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"DELETE\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}\",");
    b.append("       \"rename\": {");
    b.append("        \"param\": {");
    b.append("         \"planet_id\": \"id\"");
    b.append("        }");
    b.append("       },");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"id\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"exist\": [");
    b.append("         \"id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{id}\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    },");
    b.append("    \"update\": {");
    b.append("     \"input\": \"data\",");
    b.append("     \"name\": \"update\",");
    b.append("     \"points\": [");
    b.append("      {");
    b.append("       \"args\": {");
    b.append("        \"params\": [");
    b.append("         {");
    b.append("          \"kind\": \"param\",");
    b.append("          \"name\": \"id\",");
    b.append("          \"orig\": \"planet_id\",");
    b.append("          \"reqd\": true,");
    b.append("          \"type\": \"`$STRING`\"");
    b.append("         }");
    b.append("        ]");
    b.append("       },");
    b.append("       \"contract\": {");
    b.append("        \"id\": \"PUT /api/planet/{planet_id}\",");
    b.append("        \"json\": \"{\\\"parameters\\\":[{\\\"in\\\":\\\"path\\\",\\\"name\\\":\\\"planet_id\\\",\\\"required\\\":true,\\\"schema\\\":{\\\"type\\\":\\\"string\\\"}}],\\\"protocol\\\":\\\"http\\\",\\\"requestBody\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"forbidReason\\\":{\\\"description\\\":\\\"Why the planet is forbidden, carried from the forbid action's `why`. Absent while the planet is allowed.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"forbidState\\\":{\\\"description\\\":\\\"Set by the forbid action, and absent until it first runs. One of allowed or forbidden.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"terraformState\\\":{\\\"description\\\":\\\"Set by the terraform action, and absent until it first runs. One of idle, terraforming or complete.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"required\\\":true},\\\"responses\\\":{\\\"200\\\":{\\\"content\\\":{\\\"application/json\\\":{\\\"schema\\\":{\\\"properties\\\":{\\\"diameter\\\":{\\\"format\\\":\\\"float\\\",\\\"type\\\":\\\"number\\\"},\\\"forbidReason\\\":{\\\"description\\\":\\\"Why the planet is forbidden, carried from the forbid action's `why`. Absent while the planet is allowed.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"forbidState\\\":{\\\"description\\\":\\\"Set by the forbid action, and absent until it first runs. One of allowed or forbidden.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"},\\\"id\\\":{\\\"type\\\":\\\"string\\\"},\\\"kind\\\":{\\\"type\\\":\\\"string\\\"},\\\"name\\\":{\\\"type\\\":\\\"string\\\"},\\\"terraformState\\\":{\\\"description\\\":\\\"Set by the terraform action, and absent until it first runs. One of idle, terraforming or complete.\\\",\\\"readOnly\\\":true,\\\"type\\\":\\\"string\\\"}},\\\"required\\\":[\\\"id\\\",\\\"name\\\",\\\"kind\\\",\\\"diameter\\\"],\\\"type\\\":\\\"object\\\"}}},\\\"description\\\":\\\"OK\\\"}},\\\"securitySource\\\":\\\"unspecified\\\"}\",");
    b.append("        \"source\": \"openapi3\",");
    b.append("        \"version\": 1");
    b.append("       },");
    b.append("       \"kind\": \"http\",");
    b.append("       \"method\": \"PUT\",");
    b.append("       \"orig\": \"/api/planet/{planet_id}\",");
    b.append("       \"rename\": {");
    b.append("        \"param\": {");
    b.append("         \"planet_id\": \"id\"");
    b.append("        }");
    b.append("       },");
    b.append("       \"segments\": [");
    b.append("        {");
    b.append("         \"lit\": \"api\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"lit\": \"planet\"");
    b.append("        },");
    b.append("        {");
    b.append("         \"var\": \"id\"");
    b.append("        }");
    b.append("       ],");
    b.append("       \"select\": {");
    b.append("        \"exist\": [");
    b.append("         \"id\"");
    b.append("        ]");
    b.append("       },");
    b.append("       \"transform\": {");
    b.append("        \"req\": \"`reqdata`\",");
    b.append("        \"res\": \"`body`\"");
    b.append("       },");
    b.append("       \"parts\": [");
    b.append("        \"api\",");
    b.append("        \"planet\",");
    b.append("        \"{id}\"");
    b.append("       ]");
    b.append("      }");
    b.append("     ]");
    b.append("    }");
    b.append("   },");
    b.append("   \"relations\": {");
    b.append("    \"ancestors\": []");
    b.append("   }");
    b.append("  }");
    b.append(" }");
    b.append("}");
    return b.toString();
  }
}
