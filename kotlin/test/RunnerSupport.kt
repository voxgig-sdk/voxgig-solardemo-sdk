...
object RunnerSupport {
  ...
  fun envOverride(m: MutableMap<String, Any?>): MutableMap<String, Any?> {
    if ("TRUE" == getenv("SOLARDEMO_TEST_LIVE") || "TRUE" == getenv("SOLARDEMO_TEST_OVERRIDE")) {
    ...
    val explain = getenv("SOLARDEMO_TEST_EXPLAIN")
  ...
  fun makeCtxFromMap(ctxmapIn: MutableMap<String, Any?>?, client: SolardemoSDK?, utility: Utility?): Context {
  ...
  fun fixctx(ctx: Context?, client: SolardemoSDK?) {
