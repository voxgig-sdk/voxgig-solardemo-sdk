...
defmodule Solardemo.PrimaryUtilityTest do
  use ExUnit.Case
  ...
  defp livectx(node, opts) do
    ...
    util = Solardemo.get_utility(sdk)
    rootctx = Solardemo.get_root_ctx(sdk)
  ...
  test "the primary corpus runs through the vendored omni engine", %{run: run, sdk: sdk} do
    ...
    runsection.("makeContext", fn args ->
      ...
      out = Solardemo.Context.new(ctxmap, nil)
    ...
    specsdk = Solardemo.test(nil, setup_opts(spec, "makeSpec"))
    ...
    runsection.("operator", fn args ->
      op = Solardemo.Operation.new(argnode(args, 0))
    ...
    authsdk = Solardemo.test(nil, setup_opts(spec, "prepareAuth"))
