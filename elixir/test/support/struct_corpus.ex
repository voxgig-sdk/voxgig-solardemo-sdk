# Vendored struct corpus runner for the Solardemo SDK.
#
...
defmodule Solardemo.StructCorpus do
  alias Voxgig.Struct, as: S
  alias Voxgig.Struct.Error, as: SE
  alias Solardemo.Utility, as: U
  ...
  defp pbuild(ctxmap, util, opts, config) do
    ctx = Solardemo.Context.new(ctxmap, nil)
    ...
    if S.ismap(respmap) do
      response = Solardemo.Response.new(respmap)
  ...
  defp pclient_for(primary, name) do
    ...
    if S.ismap(setup), do: Solardemo.test(nil, S.clone(setup)), else: Solardemo.test()
  end
  ...
  def run_primary(testfile \\ nil) do
    ...
    client = Solardemo.test()
    Process.put(:pclient, client)
    Process.put(:pbaseclient, client)
    rootctx = Solardemo.get_root_ctx(client)
    ...
    run_primary_set("makeContext", psec(primary, "makeContext"), fn e ->
      ...
      Solardemo.Context.new(ctxmap, nil)
    end)
    ...
    run_primary_set("operator", psec(primary, "operator"), fn e ->
      inv = eget(e, "in")
      op = Solardemo.Operation.new(if(S.ismap(inv), do: inv, else: S.jm([])))
      S.jm([
