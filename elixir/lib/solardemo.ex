...
defmodule Solardemo do
  alias Voxgig.Struct, as: S
  alias Solardemo.Helpers, as: H
  alias Solardemo.{Utility, Context, Spec}
  ...
  def new(options \\ nil) do
    ...
    config = Solardemo.Config.shared_config()
    ...
    if S.islist(feature_order) and S.size(feature_order) > 0 do
      Enum.each(0..(S.size(feature_order) - 1), fn i ->
        ...
        if fom != nil and S.getprop(fom, "active") == true do
          Utility.feature_add(rootctx, Solardemo.Features.make_feature(fname))
        end
      end)
    end
      ...
      _ ->
        ...
        cond do
          ...
          S.ismap(fetched) ->
            ...
            content_length =
              if S.ismap(headers), do: Solardemo.Feature.header_get(headers, "content-length"), else: nil
  ...
  def graphql(client, query, variables \\ nil, ctrl \\ nil) do
    if not op_allowed?(client, "graphql") do
      ...
      if count == 0 do
        ...
        S.setprop(res, "err", "SolardemoSDK: graphql: " <> msg)
  ...
  @doc "Entity factory for moon."
  def moon(client, entopts \\ nil) do
    Solardemo.Entity.Moon.new(client, entopts)
  end

  @doc "Entity factory for planet."
  def planet(client, entopts \\ nil) do
    Solardemo.Entity.Planet.new(client, entopts)
  end
