...
use solardemo_sdk::core::helpers::{
...
/// env_override (mirrors go): when SOLARDEMO_TEST_LIVE or
/// SOLARDEMO_TEST_OVERRIDE is TRUE, environment variables replace the
/// defaults in `m` (JSON-shaped values are parsed).
pub fn env_override(m: Value) -> Value {
    let live = std::env::var("SOLARDEMO_TEST_LIVE").unwrap_or_default() == "TRUE";
    let over = std::env::var("SOLARDEMO_TEST_OVERRIDE").unwrap_or_default() == "TRUE";
    ...
    if let Ok(explain) = std::env::var("SOLARDEMO_TEST_EXPLAIN") {
        if !explain.is_empty() {
            setp(&m, "SOLARDEMO_TEST_EXPLAIN", Value::str(explain));
        }
    }
...
pub fn match_string(pattern: &str, val: &str) -> bool {
    if pattern.len() >= 2 && pattern.starts_with('/') && pattern.ends_with('/') {
        return solardemo_sdk::utility::voxgigstruct::re::Regex::new(
...
pub fn fh_has_feature(name: &str) -> bool {
    let config = solardemo_sdk::make_config();
    ...
    *ctx.result.borrow_mut() = Some(result.clone());
    ...
    let resp = Response::new(response_map);
    {
        ...
        if let Value::Func(_) = &resp.json {
            r.body = solardemo_sdk::call_json(&resp.json);
        }
