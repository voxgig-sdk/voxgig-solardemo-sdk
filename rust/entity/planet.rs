// Planet entity client (generated — mirrors the go Entity fragment).
...
impl Entity for PlanetEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        PlanetEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, SolardemoError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = PlanetEntity::new(&self.client, vs::clone(&self.entopts));
