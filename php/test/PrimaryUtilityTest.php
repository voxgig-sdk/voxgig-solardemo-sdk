...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        SolardemoSDK $client,
        SolardemoUtility $utility
    ): SolardemoContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new SolardemoContext($ctxmap, null);
    ...
    private static function fixctx(SolardemoContext $ctx, SolardemoSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?SolardemoError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SolardemoError($code, $msg);
    }
    ...
    {
        $client = SolardemoSDK::test(null, null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new SolardemoBaseFeature();
    ...
    {
        $hook_client = SolardemoSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends SolardemoBaseFeature {
            public $hook_fn = null;
            public function TestHook(SolardemoContext $ctx): void
            {
    ...
    {
        $init_client = SolardemoSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends SolardemoBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new SolardemoSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new SolardemoSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = SolardemoSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = SolardemoSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof SolardemoError) ? $result->err->msg : (string)$result->err;
