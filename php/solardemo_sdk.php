...
class SolardemoSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = SolardemoHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = SolardemoHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!SolardemoFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, SolardemoFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return SolardemoUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = SolardemoHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = SolardemoHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = SolardemoHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new SolardemoError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = SolardemoHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new SolardemoError("graphql_error",
                "SolardemoSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // PHP method names are case-insensitive, so lowercase $client->moon()
    // resolves here too.
    public function Moon($data = null)
    {
        require_once __DIR__ . '/entity/moon_entity.php';
        if ($data === null) {
            if ($this->_moon === null) {
                $this->_moon = new MoonEntity($this, null);
            }
            return $this->_moon;
        }
        return new MoonEntity($this, $data);
    }
