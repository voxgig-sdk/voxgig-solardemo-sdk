...
exports.awsparamsprovider = awsparamsprovider;
exports.gcpsecretsprovider = gcpsecretsprovider;
exports.azuresecretsprovider = azuresecretsprovider;
exports.onepasswordprovider = onepasswordprovider;
exports.dopplerprovider = dopplerprovider;
...
function envprovider(prefix, source) {
    ...
    return {
        lookup: (name) => {
            const value = env[(0, Sekreto_1.envkey)(name, prefix)];
...
function dotenvprovider(file, prefix) {
    ...
    const load = () => {
        if (undefined === values) {
            try {
                values = (0, Sekreto_1.parsedotenv)((0, node_fs_1.readFileSync)(file, 'utf8'));
            }
            catch (err) {
                ...
                if ('ENOENT' === err.code || 'ENOTDIR' === err.code) {
                    ...
                    throw new Sekreto_1.SekretoError('sekreto: dotenv provider cannot read ' + file + ': ' + err.message);
                }
            }
        }
        return values;
    ...
    return {
        lookup: (name) => load()[(0, Sekreto_1.envkey)(name, prefix)],
...
function memoryprovider(values, prefix) {
    return {
        lookup: (name) => values[(0, Sekreto_1.envkey)(name, prefix)],
...
function fileprovider(dir, prefix) {
    return {
        lookup: (name) => {
            const file = (0, node_path_1.join)(dir, (0, Sekreto_1.envkey)(name, prefix));
            let text;
            try {
                ...
                throw new Sekreto_1.SekretoError('sekreto: file provider cannot read ' + file + ': ' + err.message);
            }
...
function checkaddr(addr) {
    ...
    if (!addr.startsWith('http://')) {
        throw new Sekreto_1.SekretoError('sekreto: not an http(s) address: ' + addr);
    }
    const host = addr.slice('http://'.length).split('/')[0].split(':')[0];
    if ('localhost' === host || '127.0.0.1' === host || '::1' === host || '[::1]' === host) {
        return;
    }
    throw new Sekreto_1.SekretoError('sekreto: refusing to send a token in plaintext to ' + addr + ' (use https)');
}
...
async function fetchjson(method, url, headers, body) {
    ...
    try {
        ...
        throw new Sekreto_1.SekretoError('sekreto: cannot reach ' + url.split('?')[0] + ': ' + err.message);
    }
    ...
    try {
        ...
        if (200 === res.status) {
            throw new Sekreto_1.SekretoError('sekreto: malformed response from ' + url.split('?')[0]);
        }
    }
...
function hashicorpprovider(addr, token, options) {
    ...
    if (1 !== kv && 2 !== kv) {
        throw new Sekreto_1.SekretoError('sekreto: hashicorp: unsupported kv version: ' + String(kv));
    }
    ...
    const login = async () => {
        ...
        if (!auth) {
            throw new Sekreto_1.SekretoError('sekreto: hashicorp: no token and no auth method');
        }
        ...
        if ('kubernetes' === auth.method) {
            ...
            if (undefined === jwt) {
                ...
                try {
                    jwt = (0, node_fs_1.readFileSync)(file, 'utf8').trim();
                }
                catch (err) {
                    throw new Sekreto_1.SekretoError('sekreto: hashicorp: cannot read jwt file ' + file);
                }
            }
            ...
            throw new Sekreto_1.SekretoError('sekreto: hashicorp: unknown auth method: ' + String(auth.method));
        }
        ...
        if (200 !== res.status || !got) {
            throw new Sekreto_1.SekretoError('sekreto: hashicorp login failed: ' + res.status + ': ' + url);
        }
    ...
    return {
        lookup: async (name) => {
            ...
            const ref = (0, Sekreto_1.vaultref)(name);
            ...
            if (200 !== res.status) {
                throw new Sekreto_1.SekretoError('sekreto: hashicorp error: ' + res.status + ': ' + url);
            }
...
function boruprovider(options) {
    ...
    if (opts.addr) {
        ...
        return {
            lookup: async (name) => {
                ...
                if (200 !== res.status) {
                    throw new Sekreto_1.SekretoError('sekreto: boru serve error: ' + res.status + ': ' + url);
                }
    ...
    return {
        lookup: (name) => {
            ...
            const run = (0, node_child_process_1.spawnSync)(command, ['vault', 'get', '--reveal', alias], {
                encoding: 'utf8',
                env,
            });
            if (run.error) {
                throw new Sekreto_1.SekretoError('sekreto: cannot run ' + command + ': ' + run.error.message);
            }
            if (0 === run.status) {
            ...
            throw new Sekreto_1.SekretoError('sekreto: boru vault error: ' + (why || 'exit ' + run.status));
        },
...
function awsauth(opts) {
    ...
    if ('' === region) {
        throw new Sekreto_1.SekretoError('sekreto: aws: no region (set region or AWS_REGION)');
    }
    if ('' === keyid || '' === secret) {
        throw new Sekreto_1.SekretoError('sekreto: aws: no credentials (set keyid/secret or AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY)');
    }
...
function awssecretsprovider(options) {
    ...
    return {
        lookup: async (name) => {
            ...
            if (200 !== res.status) {
                throw new Sekreto_1.SekretoError('sekreto: aws secretsmanager error: ' + res.status);
            }
    ...
    return {
        lookup: async (name) => {
            const res = await awscall(opts, 'ssm', 'AmazonSSM.GetParameter', {
                Name: (0, Sekreto_1.awsparam)(name, opts.prefix),
                WithDecryption: true,
            });
            ...
            if (200 !== res.status) {
                throw new Sekreto_1.SekretoError('sekreto: aws ssm error: ' + res.status);
            }
...
function gcpsecretsprovider(options) {
    ...
    const login = async () => {
        ...
        const url = metadataaddr().replace(/\/$/, '') +
            '/computeMetadata/v1/instance/service-accounts/default/token';
        const res = await fetchjson('GET', url, { 'Metadata-Flavor': 'Google' });
        const got = res.body && res.body.access_token;
        if (200 !== res.status || !got) {
            throw new Sekreto_1.SekretoError('sekreto: gcp: no token and metadata server did not answer');
        }
    ...
    return {
        lookup: async (name) => {
            ...
            if ('' === project) {
                throw new Sekreto_1.SekretoError('sekreto: gcp: no project');
            }
            ...
            if (200 !== res.status) {
                throw new Sekreto_1.SekretoError('sekreto: gcp error: ' + res.status + ': ' + url);
            }
...
function azuresecretsprovider(options) {
    ...
    const login = async () => {
        ...
        if (opts.tenant && opts.clientid && opts.clientsecret) {
            ...
            const form = 'grant_type=client_credentials&client_id=' +
                encodeURIComponent(opts.clientid) +
                '&client_secret=' +
                encodeURIComponent(opts.clientsecret) +
                '&scope=' +
                encodeURIComponent(resource + '/.default');
            const res = await fetchjson('POST', url, { 'content-type': 'application/x-www-form-urlencoded' }, form);
            const got = res.body && res.body.access_token;
            if (200 !== res.status || !got) {
                throw new Sekreto_1.SekretoError('sekreto: azure login failed: ' + res.status);
            }
            renewat = expiry(res.body.expires_in);
            return String(got);
        }
        const imds = (opts.imdsaddr || 'http://169.254.169.254').replace(/\/$/, '') +
            '/metadata/identity/oauth2/token?api-version=2018-02-01&resource=' +
            encodeURIComponent(resource);
        const res = await fetchjson('GET', imds, { Metadata: 'true' });
        const got = res.body && res.body.access_token;
        if (200 !== res.status || !got) {
            throw new Sekreto_1.SekretoError('sekreto: azure: no token, no client credentials, and IMDS did not answer');
        }
        renewat = expiry(res.body.expires_in);
        return String(got);
    };
    return {
        lookup: async (name) => {
            ...
            if ('' === vault) {
                throw new Sekreto_1.SekretoError('sekreto: azure: no vault');
            }
            // Only an explicit scheme is a URL; a vault NAMED httpvault must
            // still become https://httpvault.vault.azure.net.
            const vaulturl = vault.startsWith('http://') || vault.startsWith('https://')
                ? vault
            ...
            const url = vaulturl.replace(/\/$/, '') +
                '/secrets/' +
                (0, Sekreto_1.flatname)(name, '-') +
                '?api-version=' +
                (opts.apiversion || '7.4');
            const res = await fetchjson('GET', url, { authorization: 'Bearer ' + livetoken });
            if (404 === res.status) {
            ...
            if (200 !== res.status) {
                throw new Sekreto_1.SekretoError('sekreto: azure error: ' + res.status + ': ' + url.split('?')[0]);
            }
...
function onepasswordprovider(options) {
    ...
    const resolvevault = async (addr) => {
        ...
        if ('' === want) {
            throw new Sekreto_1.SekretoError('sekreto: onepassword: no vault');
        }
        const res = await fetchjson('GET', addr + '/v1/vaults', auth());
        if (200 !== res.status || !Array.isArray(res.body)) {
            throw new Sekreto_1.SekretoError('sekreto: onepassword error: ' + res.status + ': listing vaults');
        }
        ...
        throw new Sekreto_1.SekretoError('sekreto: onepassword: no vault named ' + want);
    };
    return {
        lookup: async (name) => {
            (0, Sekreto_1.checkname)(name);
            const addr = (opts.addr || '').replace(/\/$/, '');
            if ('' === addr) {
                throw new Sekreto_1.SekretoError('sekreto: onepassword: no addr');
            }
            checkaddr(addr);
            ...
            const found = await fetchjson('GET', addr + '/v1/vaults/' + vaultid + '/items?filter=' + filter, auth());
            if (200 !== found.status || !Array.isArray(found.body)) {
                throw new Sekreto_1.SekretoError('sekreto: onepassword error: ' + found.status + ': finding ' + name);
            }
            if (0 === found.body.length) {
                return undefined;
            }
            const item = await fetchjson('GET', addr + '/v1/vaults/' + vaultid + '/items/' + found.body[0].id, auth());
            if (200 !== item.status) {
                throw new Sekreto_1.SekretoError('sekreto: onepassword error: ' + item.status + ': reading ' + name);
            }
...
function dopplerprovider(options) {
    ...
    const load = async () => {
        ...
        if (200 !== res.status || !res.body || 'object' !== typeof res.body) {
            throw new Sekreto_1.SekretoError('sekreto: doppler error: ' + res.status);
        }
        values = {};
    ...
    return {
        lookup: async (name) => (await load())[(0, Sekreto_1.envkey)(name)],
        describe: () => 'doppler' + (opts.project ? ':' + opts.project + '/' + (opts.config || '') : ''),
    };
}
/** Infisical.
 *
...
function infisicalprovider(options) {
    ...
    const login = async (addr) => {
        ...
        if (!opts.clientid || !opts.clientsecret) {
            throw new Sekreto_1.SekretoError('sekreto: infisical: no token and no client credentials');
        }
        const res = await fetchjson('POST', addr + '/api/v1/auth/universal-auth/login', { 'content-type': 'application/json' }, JSON.stringify({ clientId: opts.clientid, clientSecret: opts.clientsecret }));
        const got = res.body && res.body.accessToken;
        if (200 !== res.status || !got) {
            throw new Sekreto_1.SekretoError('sekreto: infisical login failed: ' + res.status);
        }
    ...
    return {
        lookup: async (name) => {
            ...
            if ('' === project || '' === environment) {
                throw new Sekreto_1.SekretoError('sekreto: infisical: no project/environment');
            }
            if (undefined === livetoken || Date.now() >= renewat) {
            ...
            if (200 !== res.status) {
                throw new Sekreto_1.SekretoError('sekreto: infisical error: ' + res.status);
            }
...
function makeprovider(spec) {
    switch (spec.kind) {
        ...
        default:
            throw new Sekreto_1.SekretoError('sekreto: unknown provider kind: ' + String(spec.kind));
    }
}
