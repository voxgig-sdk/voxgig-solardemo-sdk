...
require_relative 'Solardemo_types'
...
class SolardemoSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SolardemoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SolardemoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SolardemoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SolardemoUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SolardemoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SolardemoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SolardemoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = SolardemoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SolardemoError.new(
        "graphql_error", "SolardemoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Moon.list / client.Moon.load({ "id" => ... })
  def Moon(data = nil)
    require_relative 'entity/moon_entity'
    MoonEntity.new(self, data)
  end
