// moon direct API tests (generated from the API model).
...
public class MoonDirectTest
{
    ...
    {
        var setup = MoonDirectSetup(new List<object?>
        {
        ...
        if (setup.Live)
        {
            foreach (var _liveKey in new[] { "planet01" })
            {
        ...
        if (setup.Live)
        {
            pathParams["planet_id"] = setup.Idmap["planet01"];
        }
        else
        {
            pathParams["planet_id"] = "direct01";
        }
    ...
    {
        var setup = MoonDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-moon", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
    ...
    private static MoonDirectSetupResult MoonDirectSetup(object? mockres)
    {
        ...
        if (live)
        {
            ...
            return new MoonDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
