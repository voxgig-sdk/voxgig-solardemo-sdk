...
final class PrimaryUtilityTest: XCTestCase {
  ...
  private static func primaryRun() throws -> OmniResolver.Run {
    ...
    let run = try OmniResolver.makeRunner(
      SdkRunner.testJsonPath(), SolardemoSDK.testSDK(nil, nil))("primary")
    RUN = run
    return run
  }

  // XCTestCase has no custom init; set these up per test.
  private var client: SolardemoSDK!
  private var utility: Utility!

  override func setUp() {
    super.setUp()
    client = SolardemoSDK.testSDK(nil, nil)
  ...
  private func makeTestCtx(_ client: SolardemoSDK, _ utility: Utility,
  ...
  private func makeTestFullCtx(_ client: SolardemoSDK, _ utility: Utility) -> Context {
  ...
  func testFeatureHookBasic() {
    let hookClient = SolardemoSDK.testSDK(nil, nil)
  ...
  func testFeatureInitBasic() {
    let initClient = SolardemoSDK.testSDK(nil, nil)
  ...
  func testFetcherLive() throws {
    ...
    let liveClient = SolardemoSDK(opts)
  ...
  func testFetcherBlockedTestMode() {
    ...
    let opts = VMap()
    opts.entries["system"] = .map(vm(("fetch", .nat(fetch))))
    let blockedClient = SolardemoSDK(opts)
  ...
  func testMakeSpecBasic() throws {
    ...
    let specClient = SolardemoSDK.testSDK(nil, setupOpts)
  ...
  func testPrepareAuthBasic() throws {
    ...
    let authClient = SolardemoSDK.testSDK(nil, setupOpts)
