...
enum OmniResolver {
  ...
  private final class ProviderBox {
    var clients: [ObjectIdentifier: SolardemoSDK] = [:]
  ...
  private static func register(_ provider: Omni.Provider, _ client: SolardemoSDK,
    _ isdef: Bool)
  {
    providers.lock.lock()
  ...
  private static func defclient(_ provider: Omni.Provider) -> SolardemoSDK? {
    providers.lock.lock()
    defer { providers.lock.unlock() }
  ...
  private static func sdkProvider(_ client: SolardemoSDK, _ isdef: Bool = false)
    -> Omni.Provider
  {
    ...
    provider.errify = { err in
      if let sdkerr = err as? SolardemoError {
        var entries: [(String, Omni.Json)] = [
          ("name", .str("SolardemoError")),
  ...
  final class Run {
    ...
    fileprivate init(_ pack: Omni.RunPack, _ client: SolardemoSDK) {
      self.pack = pack
  ...
  /// itself).
  static func makeRunner(_ testfile: String, _ client: SolardemoSDK) throws
    -> NamedRunner
  {
  ...
  static func makeRunner(_ spec: Value, _ client: SolardemoSDK) throws
    -> NamedRunner
  {
    return try namedRunner(try Omni.makeRunner(tojson(spec), sdkProvider(client)), client)
  }

  private static func namedRunner(_ runner: Omni.Runner, _ client: SolardemoSDK)
    -> NamedRunner
  {
  ...
  static func omniCtx(_ arg: Value, _ client: SolardemoSDK, _ utility: Utility)
    -> Context
  {
