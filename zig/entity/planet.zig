// Planet entity client (generated — mirrors the go/rust Entity fragment).
...
pub const PlanetEntity = struct {
    ...
    pub fn new(client: *sdk.SolardemoSDK, entopts_in: Value) *PlanetEntity {
        ...
        const e = h.A().create(PlanetEntity) catch unreachable;
        e.* = .{
            .name = "planet",
            .client = client,
    ...
    fn ent_ctx(self: *PlanetEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *PlanetEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *PlanetEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *PlanetEntity, ctx: *Context, post_done: *const fn (*PlanetEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *PlanetEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *PlanetEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *PlanetEntity, ctx: *Context, post_done: *const fn (*PlanetEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *PlanetEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *PlanetEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return PlanetEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *PlanetEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *PlanetEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *PlanetEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *PlanetEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *PlanetEntity, ctx: *Context) void {
    ...
    pub fn list(self: *PlanetEntity, reqmatch: Value, ctrl: Value) EntListResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    pub fn create(self: *PlanetEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *PlanetEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *PlanetEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn update_post_done(self: *PlanetEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
    ...
    pub fn remove(self: *PlanetEntity, reqmatch: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *PlanetEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
