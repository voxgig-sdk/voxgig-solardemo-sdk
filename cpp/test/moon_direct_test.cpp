// Generated direct-call tests for the moon entity (unit mode;
...
static MoonDirectSetup moon_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void moon_direct_list() {
  auto setup = moon_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-moon", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "planet_id", Value("direct01"));

  Value result = client->direct(vmap({
    {"path", Value("api/planet/{planet_id}/moon")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
static void moon_direct_load() {
  auto setup = moon_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-moon", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "id", Value("direct01"));
  setp(params, "planet_id", Value("direct02"));

  Value result = client->direct(vmap({
    {"path", Value("api/planet/{planet_id}/moon/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(moon_direct_list);
  T_RUN(moon_direct_load);
  return sdktest::summary("moon_direct_test");
}
