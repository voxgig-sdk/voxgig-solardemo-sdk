...
[MCP](https://modelcontextprotocol.io) server exposing the Solardemo SDK as
two agent tools — `solardemo_list` and `solardemo_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/solardemo-mcp)
make build
...
claude mcp add --scope user solardemo \
  -- /absolute/path/to/solardemo-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./solardemo-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// solardemo_list: first page of records
{ "entity": "moon" }
{ "entity": "moon", "query": { } }

// solardemo_load: one record by id
{ "entity": "moon", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/solardemo-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user solardemo \
     -- "$PWD"/dist/*/solardemo-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `solardemo_list` and `solardemo_load` tools now appear
   in new sessions. Ask the agent to *"list moon using solardemo"*
   and it calls `solardemo_list` with `{"entity":"moon"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `solardemo_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `solardemo_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./solardemo-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"solardemo_load","arguments":{"entity":"moon","query":{"id":1}}}}'
```
