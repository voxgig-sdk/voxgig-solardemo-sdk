...
let () =
  test "moon.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.moon client Noval in
      check_str "name" ent.e_name "moon")

let () =
  test "moon.seeded_ops" (fun () ->
      let record = jo [("id", Str "moon01")] in
      let seed = jo [("moon",
                      jo [("moon01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "moon01")]) Noval in
      let loaded_data = loaded.e_data_get () in
      check "load data is a map" (ismap loaded_data);
      check_vstr "load id" (getp loaded_data "id") "moon01";
      ())

let () =
  test "moon.stream" (fun () ->
      let mk_seed () =
        jo [("moon",
      ...
      let ent2 = Sdk_client.moon client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.moon sclient Noval in
        ...
        let cent = Sdk_client.moon cclient Noval in
