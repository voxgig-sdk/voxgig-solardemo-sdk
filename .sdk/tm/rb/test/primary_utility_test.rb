...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = SolardemoBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = SolardemoSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = SolardemoSDK.test(nil, nil)
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = SolardemoSDK.new({
      "base" => "http://localhost:8080",
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
  ...
  def test_make_spec_basic
    ...
    spec_client = SolardemoSDK.test(nil, setup_opts)
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = SolardemoOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = SolardemoSDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = SolardemoContext.new(ctxmap, nil)
