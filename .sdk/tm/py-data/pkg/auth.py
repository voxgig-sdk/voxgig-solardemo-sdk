# Solardemo Data — credential discovery.
#
...
# copied verbatim per SDK — only `Solardemo` is substituted at generation
...
def missing_token_message(prefix: str, token_key: str | None = None) -> str:
    ...
    tkey = token_key or f"{prefix}_APIKEY"
    return (
        f"No API credential found for Solardemo.\n"
        f"\n"
...
def missing_base_message(prefix: str, base_key: str | None = None) -> str:
    ...
    bkey = base_key or f"{prefix}_BASE"
    return (
        f"Solardemo is multi-tenant: it has no single API host, so the base\n"
