...
defmodule Solardemo.Utility do
  alias Voxgig.Struct, as: S
  alias Solardemo.Helpers, as: H
  alias Solardemo.{Context, Spec, Result, Response, Operation}

  @default_user_agent "Mozilla/5.0 (compatible; SolardemoSDK/1.0)"
  ...
  def make_options_impl(ctx) do
    ...
    # (Solardemo.Config.shared_config) and merge uses its nested nodes as
  ...
  def make_error_impl(ctx, err) do
    ...
    errmsg =
      cond do
        match?(%Solardemo.Error{}, err) -> err.msg
    ...
    sdk_err = if match?(%Solardemo.Error{}, err), do: %{sdk_err | code: err.code}, else: sdk_err
