# Solardemo SDK generic entity behaviour
#
...
defmodule Solardemo.EntityBase do
  alias Voxgig.Struct, as: S
  alias Solardemo.Helpers, as: H
  alias Solardemo.{Context, Utility, Pipeline}
  ...
  def construct(module, client, name, entopts \\ nil) do
    ...
    S.setprop(ent, "_utility", Solardemo.get_utility(client))
    S.setprop(ent, "_entopts", entopts)
    S.setprop(ent, "_data", S.jm([]))
    S.setprop(ent, "_match", S.jm([]))

    entctx =
      Context.new(S.jm(["entity", ent, "entopts", entopts]), Solardemo.get_root_ctx(client))
