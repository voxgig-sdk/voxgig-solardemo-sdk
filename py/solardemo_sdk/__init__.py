...
from solardemo_sdk.utility import register

# Load features
from solardemo_sdk.feature.base_feature import SolardemoBaseFeature
from solardemo_sdk.features import _has_feature, _make_feature
...
class SolardemoSDK:
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("SolardemoSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Moon(self, data=None) -> "MoonEntity":
        """Entity factory: client.Moon().list() / client.Moon().load({"id": ...})."""
        from solardemo_sdk.entity.moon_entity import MoonEntity
        return MoonEntity(self, data)


    def Planet(self, data=None) -> "PlanetEntity":
        """Entity factory: client.Planet().list() / client.Planet().load({"id": ...})."""
        from solardemo_sdk.entity.planet_entity import PlanetEntity
        return PlanetEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "SolardemoSDK":
        if sdkopts is None:
            sdkopts = {}
