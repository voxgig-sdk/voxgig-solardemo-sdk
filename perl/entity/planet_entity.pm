...
# Returns the current Planet data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Planet fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; PlanetLoadMatch
...
# Returns the loaded Planet data (hashref); dies with SolardemoError
# on failure.
sub load {
...
#
# reqmatch: match filter hashref (any subset of Planet fields;
# PlanetListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Planet items as an arrayref; dies with
...
# Create a new Planet.
#
# reqdata: body data hashref (PlanetCreateData shape). ctrl: optional
# per-call control. Returns the created Planet data (hashref); dies
...
#
# reqdata: body data hashref (PlanetUpdateData shape). ctrl: optional
# per-call control. Returns the updated Planet data (hashref); dies
# with SolardemoError on failure.
sub update {
  my ($self, $reqdata, $ctrl) = @_;
  my $utility = $self->{_utility};
...
# Remove an Planet matching the given criteria.
#
# reqmatch: match criteria hashref (id/query fields; PlanetRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed Planet
