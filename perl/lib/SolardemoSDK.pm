...
sub new {
  ...
  my $utility = SolardemoUtility->new;
  ...
  my $config = SolardemoConfig::shared_config();
  ...
  if (SolardemoHelpers::is_true(
    SolardemoHelpers::gpath($self->{options}, 'feature.test.active'))) {
    $self->{mode} = 'test';
  }
  ...
  my $feature_opts = SolardemoHelpers::to_map(
    SolardemoHelpers::gp($self->{options}, 'feature')) || {};
  my $featureorder = SolardemoHelpers::gpath(
    $self->{options}, '__derived__.featureorder');
  $featureorder = [] unless Voxgig::Struct::islist($featureorder);
  for my $fname (@$featureorder) {
    my $fopts = SolardemoHelpers::to_map($feature_opts->{$fname});
    if ($fopts && SolardemoHelpers::is_true($fopts->{active})) {
      $utility->{feature_add}->($self->{_rootctx},
        SolardemoFeatures::make_feature($fname));
    }
  }

  # Add extension features.
  my $extend = SolardemoHelpers::gp($self->{options}, 'extend');
...
sub get_utility {
  my ($self) = @_;
  return SolardemoUtility->copy($self->{_utility});
}
...
sub prepare {
  ...
  my $ctrl = SolardemoHelpers::to_map(
    SolardemoHelpers::gp($fetchargs, 'ctrl')) || {};
  ...
  my $path = SolardemoHelpers::gp($fetchargs, 'path');
  $path = '' unless defined $path && !ref $path;
  my $method_val = SolardemoHelpers::gp($fetchargs, 'method');
  $method_val = 'GET' unless defined $method_val && !ref $method_val;
  my $params = SolardemoHelpers::to_map(
    SolardemoHelpers::gp($fetchargs, 'params')) || {};
  my $query = SolardemoHelpers::to_map(
    SolardemoHelpers::gp($fetchargs, 'query')) || {};
  my $headers = $utility->{prepare_headers}->($ctx);

  my $base = SolardemoHelpers::gp($opts, 'base');
  $base = '' unless defined $base && !ref $base;
  my $prefix = SolardemoHelpers::gp($opts, 'prefix');
  $prefix = '' unless defined $prefix && !ref $prefix;
  my $suffix = SolardemoHelpers::gp($opts, 'suffix');
  ...
  $ctx->{spec} = SolardemoSpec->new({
    ...
    'body' => SolardemoHelpers::gp($fetchargs, 'body'),
    'step' => 'start',
  });

  # Merge user-provided headers.
  my $uh = SolardemoHelpers::gp($fetchargs, 'headers');
...
sub _op_allowed {
  my ($self, $op) = @_;
  my $allow = SolardemoHelpers::gpath($self->{options}, 'allow.op');
...
sub _op_denied {
  ...
  return {
    'ok' => 0,
    'err' => "SolardemoSDK: $op: operation not allowed by" .
...
sub _raw_request {
  ...
  if (Voxgig::Struct::ismap($fetched)) {
    my $status = SolardemoHelpers::to_int(
      SolardemoHelpers::gp($fetched, 'status'));
    my $headers = SolardemoHelpers::gp($fetched, 'headers') || {};
    ...
    my $json_data;
    unless ($no_body) {
      my $jf = SolardemoHelpers::gp($fetched, 'json');
      if (ref $jf eq 'CODE') {
...
sub graphql {
  ...
  my $errors = SolardemoHelpers::gpath($res, 'data.errors');
  ...
  if (ref $errors eq 'ARRAY' && 0 < scalar @$errors) {
    ...
    $res->{'ok'} = 0;
    $res->{'err'} = "SolardemoSDK: graphql: $msg";
    $res->{'graphql'} = $errors;
  }
...
# Canonical facade: $client->Moon->list / ->load({ 'id' => ... })
sub Moon {
  my ($self, $data) = @_;
  require(Cwd::abs_path("$DIR/../entity/moon_entity.pm"));
  return MoonEntity->new($self, $data);
}
...
sub Planet {
  my ($self, $data) = @_;
  require(Cwd::abs_path("$DIR/../entity/planet_entity.pm"));
  return PlanetEntity->new($self, $data);
}
