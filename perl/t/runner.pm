...
sub env_override {
  my ($m) = @_;
  my $live = getenv('SOLARDEMO_TEST_LIVE');
  my $override = getenv('SOLARDEMO_TEST_OVERRIDE');
  ...
  my $explain = getenv('SOLARDEMO_TEST_EXPLAIN');
  $m->{'SOLARDEMO_TEST_EXPLAIN'} = $explain if defined $explain && '' ne $explain;
