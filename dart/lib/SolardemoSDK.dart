...
import 'entity/MoonEntity.dart';
import 'entity/PlanetEntity.dart';


export 'SolardemoTypes.dart';
export 'entity/MoonEntity.dart';
export 'entity/PlanetEntity.dart';
...
import 'Config.dart';
import 'Spec.dart';
// SolardemoEntityBase / SolardemoError / BaseFeature are re-exported below;
...
// `utility` (SolardemoTypes.dart) — exporting both from this library makes
// the name ambiguous and NO dart SDK for such an API compiles:
//
//   Error: 'Utility' is exported from both 'lib/SolardemoTypes.dart'
...
export 'SolardemoEntityBase.dart' show SolardemoEntityBase;
export 'SolardemoError.dart' show SolardemoError;
...
class SolardemoSDK {
  ...
  dynamic _opDenied(String op) {
    ...
    return {
      'ok': false,
      'err': Exception('SolardemoSDK: $op: operation not allowed by'
  ...
  Future<dynamic> graphql(String query,
    ...
    if (errors is List && 0 < errors.length) {
      ...
      res['ok'] = false;
      res['err'] = Exception('SolardemoSDK: graphql: '
  ...
  // Entity access: `client.Moon().list()` / `client.Moon().load({'id': ...})`.
  MoonEntity Moon([dynamic entopts]) {
    return MoonEntity(this, entopts);
  }


  // Entity access: `client.Planet().list()` / `client.Planet().load({'id': ...})`.
  PlanetEntity Planet([dynamic entopts]) {
    return PlanetEntity(this, entopts);
  }



  static SolardemoSDK test([dynamic testoptsarg, dynamic sdkoptsarg]) {
    ...
    final testsdk = SolardemoSDK(sdkopts);
    testsdk.mode = 'test';

    return testsdk;
  }

  SolardemoSDK tester([dynamic testopts, dynamic sdkopts]) {
  ...
  @override
  String toString() {
    return 'Solardemo ' + _utility.struct.jsonify(toJSON());
  }
}

typedef SDK = SolardemoSDK;
