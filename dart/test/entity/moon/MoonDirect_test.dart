...
void tests() {
  describe('MoonDirect', () {
    ...
    test('direct-load-moon', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-moon', true == setup['live'])) {
        return;
      }
      if (skipIfMissingIds(t, setup, ["planet01"])) {
        return;
      }
    ...
    test('direct-list-moon', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-moon', true == setup['live'])) {
        return;
      }
      if (skipIfMissingIds(t, setup, ["planet01"])) {
        return;
