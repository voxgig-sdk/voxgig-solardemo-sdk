...
func makeCtxFromMap(ctxmap map[string]any, client *SolardemoSDK, utility *Utility) *Context {
	if ctxmap == nil {
		ctxmap = map[string]any{}
	}

	ctx := NewContext(ctxmap, nil)
	...
	if respMap, ok := ctxmap["response"].(map[string]any); ok {
		ctx.Response = NewResponse(respMap)
...
func fixctx(ctx *Context, client *SolardemoSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &SolardemoError{Msg: msg, Code: code}
}

// ctxToMatchMap converts a Context to a map suitable for match comparison.
func ctxToMatchMap(ctx *Context) map[string]any {
	m := map[string]any{}
