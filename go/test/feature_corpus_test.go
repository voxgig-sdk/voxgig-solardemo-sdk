...
//
// NewSolardemoSDK, not TestSDK: the `test` feature is transport: 'base' and
...
func fcCandidates(client *sdk.SolardemoSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.SolardemoSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.SolardemoSDK, name string) any {
