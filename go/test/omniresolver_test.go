...
import (
	...
	omni "github.com/voxgig-sdk/solardemo-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SolardemoError); is {
				out := map[string]any{
					"name":    "SolardemoError",
...
func omniCtx(arg any, client *sdk.SolardemoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SolardemoSDK); is {
			client = live
