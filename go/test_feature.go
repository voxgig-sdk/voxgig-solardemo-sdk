...
func (f *TestFeature) Init(ctx *Context, options map[string]any) {
	f.client = ctx.Client
	f.options = options

	entity := toMapAny(vs.GetProp(options, "entity"))
	...
	testFetcher := func(ctx *Context, _fullurl string, _fetchdef map[string]any) (any, error) {
		...
		op := ctx.Op
		entmap := toMapAny(vs.GetProp(entity, op.Entity))
		if entmap == nil {
...
func (f *TestFeature) buildArgs(ctx *Context, op *Operation, args map[string]any) any {
	opname := op.Name
