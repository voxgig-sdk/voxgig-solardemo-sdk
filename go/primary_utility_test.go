...
func TestPrimaryUtility(t *testing.T) {
	...
	t.Run("makeError-no-throw", func(t *testing.T) {
		...
		f := false
		ctx.Ctrl.Throw = &f
		ctx.Result = NewResult(map[string]any{
			"ok":      false,
	...
	t.Run("fetcher-live", func(t *testing.T) {
		...
		liveClient := NewSolardemoSDK(map[string]any{
	...
	t.Run("fetcher-blocked-test-mode", func(t *testing.T) {
		...
		blockedClient := NewSolardemoSDK(map[string]any{
			"system": map[string]any{
				"fetch": func(url string, fetchdef map[string]any) (map[string]any, error) {
	...
	t.Run("makeResult-no-result", func(t *testing.T) {
		ctx := makeTestFullCtx(client, utility)
		ctx.Spec = NewSpec(map[string]any{"step": "start"})
		ctx.Result = nil
	...
	t.Run("makeSpec-basic", func(t *testing.T) {
		...
		specClient := TestSDK(nil, setupOpts)
	...
	t.Run("prepareAuth-basic", func(t *testing.T) {
		...
		authClient := TestSDK(nil, setupOpts)
...
func (f *testHookFeature) TestHook(ctx *Context) {
	if f.hookFn != nil {
		f.hookFn()
	}
}
...
func (f *testInitFeature) Init(ctx *Context, options map[string]any) {
	if f.initFn != nil {
		f.initFn()
	}
}
...
func makeTestCtx(client *SolardemoSDK, utility *Utility, overrides map[string]any) *Context {
	ctxmap := map[string]any{
		"opname":  "load",
		"client":  client,
		"utility": utility,
	...
	return utility.MakeContext(ctxmap, client.rootctx)
}

// Helper: create full test context with target and match
func makeTestFullCtx(client *SolardemoSDK, utility *Utility) *Context {
	ctx := makeTestCtx(client, utility, nil)
	ctx.Target = map[string]any{
		"parts":     []any{"items", "{id}"},
